...
1. **pi drafts v0.1** — routing notes (NOT vault content: open questions, vocabulary choices, epistemics with confidence + n-counts, anchor specs) + PARTs written as vault citizens: edits with explicit insertion anchors **verified against the live file** (state exact line numbers; pages evolve — re-verify, never trust prior numbers; grep markers with **variant-tolerant patterns** — "(superseded)" vs "(now superseded)" cost a gate-round F-1 once), one PART for any NEW page (frontmatter complete; **every `sources:` path existence-checked**; avoid falsifiable counts in permanent prose), one PART for the todos/ page (Progress entry + checked task). Never include enumerable counts that drift; cite the receipt instead. **Derived values — post-write line counts, byte sizes, computed totals — get the same discipline as anchors: re-verify against disk or omit.** A stated-but-unverified number in a package is a falsifiable claim shipped to another desk (caught twice cross-desk 2026-09-15: stale PART-A line refs, miscounted 94-vs-97 page lines — the receiving desk's oracle caught both).
...
6. **Gate 3 — archivist single write pass:** all ops in ONE pass (PARTs + index entry + log entry; README stats footer in-pass — TNT precedent). Execution receipt: per-PART verbatim verdicts, exact final frontmatter, deviations (target zero; REPORTED changes never silent).
   ...
   **Execution Form (doctrine — verbatim, locked with ddd 2026-09-13):**
   ...
   Codified: execution form = `task(subagent_type="archivist")` — the orchestrator delegates, the persona lands every byte; in-session role-form = documented fallback only, with the full procedural envelope; every execution receipt records the form used. Verbatim-quote discipline applies: any rewording of the clause = re-arbitration, not editorial. (TNT-side landing 2026-09-15: vault-ops SKILL.md L100-108 + agents/archivist.md L58-68 + vault log one-liner via the persona itself — receipt `tnt-20260915-doctrine-clause-receipt.md`.)
