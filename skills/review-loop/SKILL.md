...
- **Category sets the model** (tier-map is the sole model authority): review =
  `unspecified-high`; deep trace = `deep`; recon = `quick` (FREE); web research = `research`. Pick the cheapest tier that
  does the job — flash-first for verifiable mechanics, flagships locked at judging nodes.
