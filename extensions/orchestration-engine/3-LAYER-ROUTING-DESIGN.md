...
- After the promo sunset (**2026-09-30** — corrected to match `tier-map.ts PROMO_SUNSET_ISO`; this doc earlier mis-stated 10-30), `glm-5.2` silently goes from 1× to 2× off-peak with no behavior change in the router. *(Update 2026-09-08: promo text removed from /devpack/overview; `PROMO_SUNSET_ISO` now pinned 2026-09-07 → promo inactive.)*
...
> **🔄 RE-VERIFY 2026-08-05 (PORT-PLAN-v0.40 §③ — ordered fallback arrays; commit `8b8c7bc`):** the single per-tier `fallbackProvider`/`fallbackId` is **superseded by an ordered cross-provider array** `fallbackModels: {provider,id}[]` on every `TierEntry`. A new pure helper `orderedFallbacks(primaryFlag, tierFallbacks, globalFallbackFlag, exclude=[])` (tier-map.ts) builds the deduped retry chain = per-tier array then the global `FALLBACK` tail (`opencode-go/glm-5.1`), with the primary + any excluded models removed. `resolveAndSpawn` (spawn.ts) now walks this chain on **both** triggers: (a) **unavailable primary** → picks the first available candidate (was: leapt straight to the global `FALLBACK`, ignoring the per-tier array); (b) **empty output** → spawns each available candidate in order until one is non-empty (was: retried *exactly once*), summing `usage`/`elapsedMs`/`toolCount` across all hops. `ResolvedModel.fallbackFlag?:string` → `fallbackFlags:string[]`. The 3 judging categories (`deep`/`ultrabrain`/`unspecified-high`) carry **2-element** arrays; the **strong-model-at-judging invariant holds** — every judging primary, array entry, and the global tail is glm-5.x/kimi (data-layer guard in `tests/fallback-resolver.test.ts`). 7 of 10 categories remain Z-AI-plan-primary. The "retries once" / single-fallback prose in the 2026-07-11 + 2026-08-04 blocks above is preserved as the point-in-time record; the realized behavior is the ordered multi-hop walk described here.
