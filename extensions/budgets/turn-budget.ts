// extensions/budgets/turn-budget.ts — turn-budget resolution, state, and termination-decision logic.
// Faithful port from pi-subagents@0.40 src/runs/shared/turn-budget.ts (verified 2026-08-04).
...
//  - resolveTurnBudgetConfig is the validator/resolver: raw config → ResolvedTurnBudget | error.
//    Fail-safe: invalid input returns {error}, never throws. Callers decide how to surface errors.
//  - turnBudgetDecision implements termination-at-boundary: a turn over the hard limit (maxTurns +
//    graceTurns) is ABORTED only at a safe assistant boundary; if tool work is active/starting and
//    hard-limit isn't force-enforced, it DEFERS to the next boundary (never kills mid-write).
//    This boundary semantics is the robustness core — ported exactly, do not simplify to "kill now".
...
export function appendTurnBudgetSystemPrompt(
  ...
  const graceTurns = budget.graceTurns ?? DEFAULT_TURN_BUDGET_GRACE_TURNS;
  const grace =
    graceTurns === 1 ? "1 additional assistant turn" : `${graceTurns} additional assistant turns`;
  const block = [
    "## Turn budget",
...
export function turnBudgetSoftNote(budget: ResolvedTurnBudget, turnCount: number): string {
  return `Turn budget wrap-up was requested after ${turnCount} assistant turn${
    turnCount === 1 ? "" : "s"
  } (soft limit ${budget.maxTurns}, grace ${budget.graceTurns}). Process-mode live steering is unavailable, so the child was warned at launch to wrap up by this budget. Output may be partial.`;
}

export function turnBudgetExceededMessage(budget: ResolvedTurnBudget, turnCount: number): string {
  return `Subagent exceeded turn budget after ${turnCount} assistant turn${
    turnCount === 1 ? "" : "s"
  } (soft limit ${budget.maxTurns} + grace ${budget.graceTurns}).`;
}

export function turnBudgetDeferredNote(budget: ResolvedTurnBudget, turnCount: number): string {
  return `Turn-budget termination was deferred at ${turnCount} assistant turn${
    turnCount === 1 ? "" : "s"
  } (soft limit ${budget.maxTurns} + grace ${budget.graceTurns}) because the assistant started tool work. The run ended before another safe assistant boundary; output may be partial.`;
}
...
/**
 * Termination decision for the turn budget.
 * - "continue": within the hard limit (maxTurns + graceTurns), or at a terminal stop already.
 * - "defer": over the hard limit BUT tool work is active/starting and hard-limit isn't forced —
 *   wait for the next safe assistant boundary rather than killing mid-write.
 * - "abort": over the hard limit at a safe boundary (or hard-limit forced) — terminate now.
 *
 * The defer branch is the robustness core: never destroy in-flight work. Ported exactly.
 */
