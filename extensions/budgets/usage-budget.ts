// extensions/budgets/usage-budget.ts — usage-budget (tokens/cost) resolution and state.
// Faithful port from pi-subagents@0.40 src/runs/shared/usage-budget.ts (verified 2026-08-04).
// Part of the budget subsystem (PORT-PLAN-v0.40.md ①). Pure library — no spawn wiring yet.
//
// DESIGN (see PORT-PLAN §①):
//  - This is the ONE budget safe to apply session-wide: usageBudgetState never stops a running
//    child — `exhausted` only blocks LATER launches. There is NO reservation model (upstream is
//    explicit: "no reservation estimates", source: "reported"). Reported usage only.
//  - validateUsageBudgetConfig: fail-safe, returns {error}; requires at least one of tokens/costUsd.
//  - usageBudgetState computes per-metric outcomes (within/soft-exceeded/hard-exceeded) and a
//    single `exhausted` flag + `reason` (tokens or costUsd, tokens win on a tie).
...
function validateLimit(
  value: unknown,
  label: string,
): { limit?: UsageBudgetLimitConfig; error?: string } {
  ...
  if (typeof raw.hard !== "number" || !Number.isFinite(raw.hard) || raw.hard <= 0)
    return { error: `${label}.hard must be a positive number.` };
  if (
    raw.soft !== undefined &&
    (typeof raw.soft !== "number" || !Number.isFinite(raw.soft) || raw.soft <= 0)
  )
    return { error: `${label}.soft must be a positive number.` };
  if (typeof raw.soft === "number" && raw.soft > raw.hard)
    return { error: `${label}.soft must be less than or equal to ${label}.hard.` };
  return { limit: { ...(typeof raw.soft === "number" ? { soft: raw.soft } : {}), hard: raw.hard } };
}

export function validateUsageBudgetConfig(
  value: unknown,
  label = "usageBudget",
): { budget?: UsageBudgetConfig; error?: string } {
  ...
  if (!budget.tokens && !budget.costUsd)
    return { error: `${label} must include tokens or costUsd.` };
  return { budget };
}
...
function metricState(
  ...
  return {
    ...
    outcome:
      used >= limit.hard
        ? "hard-exceeded"
        : limit.soft !== undefined && used >= limit.soft
          ? "soft-exceeded"
          : "within-budget",
  };
}
...
export function usageBudgetState(
  ...
  const reason =
    tokens?.outcome === "hard-exceeded"
      ? "tokens"
      : costUsd?.outcome === "hard-exceeded"
        ? "costUsd"
        : undefined;
  return {
    version: 1,
